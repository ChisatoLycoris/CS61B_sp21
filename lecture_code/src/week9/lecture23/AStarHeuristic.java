...
public class AStarHeuristic {
    ...
    public AStarHeuristic(DirectedGraphWithEdgeLabeled G, int s, int g) {
        this.start = s;
        this.goal = g;
    ...
    /**
     * Dijkstra's algorithm only consider the distance from start to v
     * A* consider both distance from start to v and distance from v to goal
     */
    private void bfs(DirectedGraphWithEdgeLabeled G, int v, PriorityQueue<TotalDistance> fringe) {
        marked[v] = true;
        while (fringe.peek().v != goal) {
            ...
            for (int w : G.adj(cur.v).keySet()) {
                ...
                if (!marked[w] && d < distTo[w]) {
                    ...
                    fringe.add(new TotalDistance(w, d + heuristic(w, goal)));
                    distTo[w] = d;
                    edgeTo[w] = cur.v;
                }
            }
    ...
    /**
     * return the estimate of the distance from v to g
     * just a simple example, in reality this function may need its own algorithm
     * @param v vertices
     * @param g goal
     */
    private int heuristic(int v, int g) {
        return (int) Math.pow(g - v, 2);
    }
    ...
    public static void main(String[] args) {
        ...
        g.addEdge(4, 6, 5);
        g.addEdge(4, 5, 4);
        AStarHeuristic aStarHeuristic = new AStarHeuristic(g, 0, 6);
        System.out.println(Arrays.toString(aStarHeuristic.edgeTo));
        System.out.println(Arrays.toString(aStarHeuristic.distTo));
        System.out.println("The result of \"edgeTo\" \"distTo\" arrays are different from Dijkstra's Algorithm");
        System.out.println("However, the result of \"shortest path to goal\" and \"its distance\" is correct");
        System.out.println("And it is faster because it doesn't go through all possibility");
    }
}
